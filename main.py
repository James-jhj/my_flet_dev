...
if IS_WINDOWS:
    ...
    # ========== 导入通知模块（新库 flet-android-notifications） ==========
    try:
        ...
        print("flet-android-notifications 导入成功")
    except ImportError as e:
        ANDROID_NOTIFY_AVAILABLE = False
        print(f"[通知] flet-android-notifications 导入失败: {e}")

        # 空占位类，电脑Windows运行兼容
        class AndroidNotification:
            def __init__(self, title="", message="", notification_id=0):
                self.title = title
                self.message = message
        ...
        def create_notification_channel(**kwargs):
            pass
        def is_notifications_enabled():
            return False
        def request_notifications_permission():
            return False

# 通知渠道常量（统一管理，不要到处写字符串）
...
async def request_notify_permission(page) -> bool:
    """主动请求通知权限（用户手动触发调用，推荐绑定按钮）
    返回 True=授权成功 False=拒绝
    """
    if not ANDROID_NOTIFY_AVAILABLE:
        return False
    ...
    # 弹出系统标准权限询问框【允许/拒绝】，不会直接跳设置！
    granted = await request_notifications_permission()
    return granted
...
def main(page: ft.Page):
        ...
        if platform.system() == "Linux":
            try:
                ...
                if not Environment.isExternalStorageManager():
                    print("[权限] 请求 MANAGE_EXTERNAL_STORAGE 权限")
                    intent = Intent(
                    ...
                    print("[权限] 已有 MANAGE_EXTERNAL_STORAGE 权限")
            except Exception as e:
                print(f"[权限] 请求失败: {e}")
    ...
    def show_notification(page, title: str, message: str, notification_id: int = None, ongoing: bool = False):
        ...
        Args:
            ...
            title: 通知标题
            message: 通知内容
            notification_id: 通知ID，相同ID会覆盖旧通知
            ongoing: 是否持续常驻通知
        """
        ...
        # 检查权限，如果没有权限直接放弃发送（不会自动跳转设置）
        if not is_notifications_enabled():
            print("[通知] 未授予通知权限，放弃推送")
            return False
    ...
    def cancel_notification(notification_id: int):
        """取消指定ID通知（适配 flet-android-notifications）"""
        ...
        if not ANDROID_NOTIFY_AVAILABLE:
            print("[通知] 通知模块不可用，跳过取消")
            return

        try:
            # 方式1：直接构造通知对象调用cancel
            n = AndroidNotification(notification_id=notification_id)
            n.cancel()
    ...
    def show_fullscreen_lyrics():
        ...
        try:
            if hasattr(page, 'window_height') and page.window_height:
                is_android = platform.system() == "Linux"
                if is_android:
                    list_view_height = page.window_height - 240
                else:
        ...
        # 计算可用宽度（减去左右边距）
        available_width = list_view_height * 0.8  # 估算可用宽度
        ...
        for i, (time_sec, text) in enumerate(current_lyrics):
            font_size = get_lyric_font_size(text)
        ...
        lyrics_fullscreen_container = ft.Container(
            ...
            right=0,
            bottom=0,
            on_click=lambda e: close_fullscreen_lyrics(user_closed=True),
        )
        ...
        lyrics_fullscreen_container.data = {
            ...
            'flash_task': None,
            'available_width': available_width,  # 保存可用宽度
        }
        ...
        async def scroll_to_lyric_index(real_index, duration=300):
            ...
            # ========== 检查容器是否存在 ==========
            if not lyrics_fullscreen_container:
                return
            ...
            try:
                ...
                # ========== 再次检查容器是否存在 ==========
                if lyrics_fullscreen_container:
    ...
    def play_music(sound_file, loop=False, event_name=None, event_id=None):
        ...
        global music_section_container, lyrics_fullscreen_container
        
        print(f"[play_music] 接收到参数 - event_name: {event_name}, event_id: {event_id},sound_file: {sound_file}")

        # ========== 关键修复：如果全屏歌词容器存在但已损坏，重新初始化 ==========
        if lyrics_fullscreen_container and lyrics_fullscreen_container not in page.overlay:
            print("[play_music] 全屏歌词容器已不在页面中，重置引用")
            lyrics_fullscreen_container = None
        ...
        def on_state_change(e):
            ...
            global auto_fullscreen_lyrics, auto_scroll_task  # 添加这行
            ...
            if e.state == AudioState.PLAYING:
                ...
                if should_loop and is_playing_new:
                    ...
                    # ========== 关键修复：记录需要自动打开全屏歌词 ==========
                    if lyrics_fullscreen_container and lyrics_fullscreen_container in page.overlay:
                        print("[循环播放] 全屏歌词正在显示，标记需要重新打开")
                        # 不要关闭，保持容器，但重置滚动状态
                        if lyrics_fullscreen_container.data:
                            lyrics_fullscreen_container.data['is_scrolling'] = False
                            # 重置歌词项高亮
                            for item in lyrics_fullscreen_container.data.get('lyric_items', []):
                                if hasattr(item.content, 'color'):
                                    item.content.color = ft.Colors.GREY_700
                                    item.content.weight = ft.FontWeight.NORMAL
                                item.bgcolor = None
        ...
        def on_position_change(e):
            ...
            if e.position is not None:
                ...
                # ========== 如果容器为空但任务是活动的，停止任务 ==========
                if not lyrics_fullscreen_container and auto_scroll_task:
                    auto_scroll_task.cancel()
                    auto_scroll_task = None
        ...
        def build_normal_mode():
            ...
            def open_memo_edit_dialog(note_id=None):
                ...
                def show_image_info(file_path):
                    ...
                    show_bottom_message(f"📄 {file_name} | {resolution_str} | {size_str}")
                ...
                def open_attachment_file(file_path):
                    ...
                    if ext in ['.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp']:
                        show_fullscreen_image(file_path)
                        return
                    ...
                    def save_to_root_silent(file_path):
                        ...
                        try:
                            ...
                            if uri:
                                ...
                                show_bottom_message(f"✅ 已保存到 Download: {file_name}")
                                return True
                        except Exception as e:
            ...
            content=ft.Column([
                ...
                ft.Row([
                    ft.Container(
                        ...
                        padding=ft.Padding(left=15, right=0, top=4, bottom=4),
                        expand=True,
                    ),
                    ft.Container(
